...
function xmldb_block_xp_upgrade($oldversion) {
    ...
    if ($oldversion < 2014031105) {

        // Define table block_xp_config to be created.
        $table = new xmldb_table('block_xp_config');

        // Adding fields to table block_xp_config.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('courseid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        $table->add_field('enablelog', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, null);
        $table->add_field('keeplogs', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        $table->add_field('levels', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);

        // Adding keys to table block_xp_config.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));

        // Conditionally launch create table for block_xp_config.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Xp savepoint reached.
        upgrade_block_savepoint(true, 2014031105, 'xp');
    }

    if ($oldversion < 2014031106) {

        // Define field lastlogpurge to be added to block_xp_config.
        $table = new xmldb_table('block_xp_config');
        $field = new xmldb_field('lastlogpurge', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0', 'levels');

        // Conditionally launch add field lastlogpurge.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Xp savepoint reached.
        upgrade_block_savepoint(true, 2014031106, 'xp');
    }
